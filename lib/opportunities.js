...
// Table name for opportunities (renamed from prover_prospects)
const TABLE_NAME = process.env.OPPORTUNITIES_TABLE || 'prover_opportunities';
...
export async function createOpportunity(workspaceId, data) {
  ...
  // Auto-generate opportunity_name if not provided
  if (!cleaned.opportunity_name && cleaned.company_name) {
    const parts = [cleaned.company_name];
    if (cleaned.contact_name) parts.push(cleaned.contact_name);
    cleaned.opportunity_name = parts.join(' - ');
  }
