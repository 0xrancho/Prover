    ...
    if (r.tool === 'web_search') {
      ...
      if (data.results && data.results.length > 0) {
        const resultList = data.results.slice(0, 5).map(sr =>
          `  - ${sr.title || 'Unknown'}: ${sr.url || 'no url'}`
        ).join('\n');
...
If incomplete (be SPECIFIC about gaps, use actual company names):
{
  "complete": false,
  "gaps": [
    "Search web for Wabash College URL and industry",
    "Search web for RetroFit Design URL and industry",
    "Update Wabash College in CRM with enrichment data",
    "Update RetroFit Design in CRM with enrichment data"
  ]
}`;
