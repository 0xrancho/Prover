...
function serializeResultsForEval(results) {
  return results.map(r => {
    ...
    // CRM read - show full opportunity list with all non-null fields
    if (r.tool === 'crm_read' && data.opportunities) {
      const opportunityList = data.opportunities.map(opp => {
        const fields = [`name: ${opp.company_name}`];
        // Dynamically show all non-null, non-system fields
        const skipFields = ['id', 'workspace_id', 'created_at', 'updated_at', 'company_name'];
        Object.entries(opp).forEach(([key, value]) => {
          if (skipFields.includes(key)) return;
          if (value === null || value === undefined) {
            fields.push(`${key}: MISSING`);
...
async function generatePlan(message, scratchpad, conversationHistory = [], replanContext = null) {
  const systemPrompt = await buildPlannerSystemPrompt(); // Now async for dynamic schema
...
function summarizeResult(tool, result) {
  ...
  switch (tool) {
    ...
    case 'crm_read':
      return `${result.opportunities?.length || 0} opportunities`;
    case 'crm_write':
...
When you notice opportunities with missing data (null URLs, null industry, etc.), proactively suggest enrichment:
