 ...
 */
export function startTurn(scratchpad, message) {
  return {
    ...scratchpad,
    turn_count: scratchpad.turn_count + 1,
    last_query: {
      message,
      tools_used: [], // Will be filled after execution
...
export function getContextSummary(scratchpad) {
  ...
  if (scratchpad.last_query) {
    const toolsInfo = scratchpad.last_query.tools_used?.length
      ? ` [${scratchpad.last_query.tools_used.join(', ')}]`
      : '';
    parts.push(`Last query${toolsInfo}: "${scratchpad.last_query.message}"`);
  }
